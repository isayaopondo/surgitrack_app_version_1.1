...
class MY_Model extends CI_Model
{
     ...
     */
    public function facilities_query($user_id, $called_during_auth = FALSE,$facility='')
    {
         $facl = [];
        ...
        if (isset($facility) && $facility != null) {
            $this->db->where("f.facility_id", $facility);
    ...
    public function multi_facilities_query($user_id, $called_during_auth = FALSE)
    {
        $multi_facl = '0';
        // Selected user table data
        $selected_columns = [
    ...
    public function get_mydefault_department($user_id, $facility)
    {
        $department = [];
        $this->db->select('u.department_id,d.department_name')
        ...
        {
            $department = ['department_id'=>'none','department_name'=>'none'];
        }
        return $department;
