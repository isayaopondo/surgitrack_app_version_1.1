...
class Ion_auth_model extends CI_Model {
     ...
     * @author Mathew
     * */
    public function register($identity, $mypassword, $email, $additional_data = array(), $groups = array()) {
        ...
        $password = $this->hash_password($mypassword, $salt);
        // Users table.
        ...
        $this->filemanager->create_root_folder($id);
        $this->trigger_events('post_register');
         $this->send_activation_mail($mypassword, $id, $additional_data['first_name'], $email, $activationcode);
        return (isset($id)) ? $id : FALSE;
    }

    function send_activation_mail($passcode, $user_id, $fname, $email, $activationcode) {
        ...
        $info = array(
            ...
            'id' => $user->user_id,
            'email' => $email,
            'new_email_key' =>  $user->activation_code,
            'password' => $passcode,
            'user_id' => $user->user_id,
            'username' => $user->first_name,
            'site_name' => SYSTEM_NAME,
            'activation_period' => 24,
        );
        $this->notificationmanager->sendMail($user_id, 'activate', "Welcome to " . SYSTEM_NAME, $email, $info);
    }
    ...
    public function login($identity, $password, $remember = FALSE) {
        ...
        $this->trigger_events('extra_where');
        $query = $this->db->select($this->identity_column . ', email, user_id, password, active, last_login,first_name,last_name,photo')
                ->where($this->identity_column, $identity)
                ->limit(1)
                ->order_by('user_id', 'desc')
                ->get($this->tables['users']);
    ...
    public function social_login($users, $identity) {
        $user = (object) $users;
        $this->set_session($user);
        $this->update_last_login($user->user_id);
        $this->clear_login_attempts($identity);
    ...
    public function unlock($password, $remember) {
        $this->trigger_events('pre_login');
        if (empty($password)) {
            $this->set_error('unlock_unsuccessful');
            return FALSE;
        }
        ...
        if ($query->num_rows() === 1) {
            ...
            if ($password === TRUE) {
                if ($user->
                        active == 0) {
                    $this->trigger_events('post_unlock_unsuccessful');
                    $this->set_error('unlock_unsuccessful_not_active');
                    return FALSE;
                } $this->set_session($user);
     ...
     * exceeded lo
      gin attempts within a given period
     *
     * @return	boolean
    ...
    public function get_last_attempt_time($identity) {
        if ($this->config->item('track_login_attempts', 'ion_auth')) {
            ...
            if ($this->config->item('track_login_ip_address', 'ion_auth'))
                ...
                $this->db->or_where('login', $identity);
            $qres = $this
                    ->db->get($this->tables['login_attempts'], 1);
            if ($qres->num_rows() > 0) {
    ...
    , $expire_period = 86400) {
        if ($this->config->item('track_login_attempts', 'ion_auth')) {
            $ip_address = $this->_prepare_ip($this->input->ip_address());
            $this->db->where(array(
                'ip_address' => $ip_address, 'login' => $identity));
...
// if group name was used we do on e more join with groups
...
// if no id was passed use the current users id
        $id || $id = $this->session->userdata('user_id');
        return $this->db->select($this->tables['users_groups'] . '.' . $this->join['groups'] . ' as id, ' . $this->tables['groups'] . '.name, ' . $this->tables['groups'] . '.description')->where($this->tables['users_groups'] . '.' . $this->join['users'], $id)->join($this->tables['groups'], $this->tables['users_groups'] . '.' . $this->join['groups'] . '=' . $this->tables['groups'] . '.group_id')->get($this->tables['users_groups']);
    }
     ...
     * @author Isaya
     * */
    public function get_users_subscriptions($id = FALSE) {
        $this->trigger_events('get_users_subscriptions');
// if no id was passed use the current users id
        $id || $id = $this->session->userdata('user_id');
        return $this->db->select($this->tables['users_groups'] . '.' . $this->join['groups'] . ' as id, ' . $this->tables['groups'] . '.name, ' . $this->tables['groups'] . '.description')->where($this->tables['users_groups'] . '.' . $this->join['users'], $id)->join($this->tables['groups'], $this->tables['users_groups'] . '.' . $this->join['groups'] . '=' . $this->tables['groups'] . '.group_id')->get($this->tables['users_groups']);
    }
...
// otherwise use what is set
    ...
    public function set_session($users) {
        ...
        $session_data = array(
            'identity' => $user->email, //{$this->identity_column}, $this->identity_column => $user->{$this->identity_column},
            'email' => $user->email,
            'user_id' => $user->user_id, //
            'fullname' => $user->first_name . ' ' . $user->last_name,
            'lastname' => $user->last_name,
            'profilephoto' => isset($user->photo) ? $user->photo : '',
...
// otherwise use what is set
            ...
            set_cookie(array(
                'name' => $this->config->item('identity_cookie_name', 'ion_auth'), 'value' => $user->{$this->identity_column},
                'expire' => $expire));
            set_cookie(array(
...
// get the user
        $this->trigger_events('extra_where');
        $query = $this->db->select($this->identity_column . ', user_id, email, last_login,photo')->where($this->identity_column, get_cookie($this->config->item('identity_cookie_name', 'ion_auth')))->where('remember_code', get_cookie($this->config->item('remember_cookie_name', 'ion_auth')))->limit(1)->order_by('user_id', 'desc')->get($this->tables['users']);
...
// extend the users cookies if the option is enabled
            ...
            $this->trigger_events(array(
                'post_login_remembered_user', 'post_login_remembered_user_successful'));
            return TRUE;
        }
        $this->trigger_events(array('post_login_remembered_user', 'post_login_remembered_user_unsuccessful'));
        return FALSE;
...
// remove all users from this group
        $this->db->delete($this->tables['users_groups'], array($this
